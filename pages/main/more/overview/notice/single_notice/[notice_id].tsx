...
import {
  notices,
  SingleNotice as SingleNoticeType,
} from "../../../../../../dummyData/notice";

interface Props {
  notice: SingleNoticeType;
}

const OverviewChildren: React.FC<Props> = ({ notice }) => (
  <div className={`${styles["notice-container"]} single-notice-container`}>
    <style jsx>{`
...
const MySingleNotice: React.FC<Props> = ({ notice }) => {
  const OverviewLayout = withOverviewLayout<Props>({
    component: OverviewChildren,
  });

  return <OverviewLayout notice={notice} />;
};

const SingleNotice = ({ notice }: Props) => {
  const ProductsLayout = withProductsLayout<Props>({
    component: MySingleNotice,
  });

  return <ProductsLayout notice={notice} />;
};
...
export const getStaticProps: GetStaticProps = ({ params }) => {
  const notice: SingleNoticeType = notices.find(
    (n) => n.id === params.notice_id
  );
